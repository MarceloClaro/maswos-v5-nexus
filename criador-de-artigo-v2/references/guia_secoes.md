...
## 横贯性规则
1. 所有引用须添加可审计脚注（格式见citacoes_auditaveis.md）
2. 每段须有6句结构：主题句-展开-证据-分析-深化-连接
3. 每章节开篇须有宣告其目的和与前章关系的过渡段
4. 每个技术术语首次出现时须定义（含类比）
5. 每条脚注须 explicitar a autoridade da fonte, sua funcao no paragrafo e sua relevancia para o problema da pesquisa
